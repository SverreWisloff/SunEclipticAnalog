...
class SunEclipticAnalogWatch extends Application.AppBase {
    ...
    public function getInitialView() as [Views] or [Views, InputDelegates] {
        if (WatchUi has :WatchFaceDelegate) {
            var view = new $.SunEclipticAnalogView();
            var delegate = new $.SunEclipticAnalogDelegate(view);
            return [view, delegate];
        } else {
            return [new $.SunEclipticAnalogView()];
        }
    }
    ...
    public function getGoalView(goal as GoalType) as [View]? {
        return [new $.SunEclipticAnalogGoalView(goal)];
    }
